...
case class MissionUser(missionUserId: Int, missionId: Int, userId: String, completed: Option[Boolean])
...
class MissionUserTable(tag: Tag) extends Table[MissionUser](tag, Some("sidewalk"), "mission_user") {
  ...
  def completed = column[Option[Boolean]]("completed", O.Nullable)

  def * = (missionUserId, missionId, userId, completed) <> ((MissionUser.apply _).tupled, MissionUser.unapply)
...
object MissionUserTable {
  ...
  /**
    ...
    */
  def save(missionId: Int, userId: String): Int = save(MissionUser(0, missionId, userId, Some(false)))
